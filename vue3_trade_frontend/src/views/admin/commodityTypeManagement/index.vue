<template>
  <div class="commodity-type-admin">
    ...
    <el-card style="margin-bottom: 10px">
      <el-row :gutter="10">
        <el-col :span="6">
          <el-form-item label="账号类别ID" label-width="80px">
            <el-input v-model="queryParams.id" placeholder="请输入账号类别ID" />
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="账号类别名称" label-width="80px">
            <el-input
              v-model="queryParams.typeName"
              placeholder="请输入账号类别名称"
            />
          </el-form-item>
        </el-col>
        <el-col :span="8">
    ...
    <el-card>
      <el-table
        ...
        :loading="loading"
      >
        <el-table-column prop="id" label="账号类别ID" />
        <el-table-column prop="typeName" label="账号类别名称" />
...
const editCommodityType = async () => {
  try {
    ...
    if (res.code === 200) {
      ElMessage.success("修改账号类别成功");
      editDialogVisible.value = false;
      await getCommodityTypeList();
    } else {
      ElMessage.error("修改账号类别失败");
    }
  } catch (error) {
    ElMessage.error("修改账号类别失败");
  }
...
const addCommodityType = async () => {
  try {
    ...
    if (res.code === 200) {
      ElMessage.success("添加账号类别成功");
      addDialogVisible.value = false;
      await getCommodityTypeList();
    } else {
      ElMessage.error("添加账号类别失败");
    }
  } catch (error) {
    ElMessage.error("添加账号类别失败");
  }
...
const deleteCommodityType = async (row) => {
  try {
    ...
    if (res.code === 200) {
      ElMessage.success("删除账号类别成功");
      await getCommodityTypeList();
    } else {
      ElMessage.error(res.message || "删除账号类别失败");
    }
  } catch (error) {
    ElMessage.error(error.message || "删除账号类别失败");
  }
};
